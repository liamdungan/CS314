...
%%%% project 2, CS 314, Fall, 2017.  Prof. Steinberg 
...
state_struct(Name, States, state(Name, Tran, Temp) ):- member( state(Name, Tran, Temp), States).
...
next_state_name(state(_, Tran, _), [Head|Tail], Next_name, Tail):- member( transition(Head, Next_name), Tran).
...
accepts(State_struct, Fsa, Chars):- 
	next_state_name(State_struct, Chars, Next_name, Tail),
	state_struct(Next_name, Fsa, Next_state),
	accepts(Next_state, Fsa, Tail).
